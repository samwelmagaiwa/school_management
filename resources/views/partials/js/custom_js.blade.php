<script>
    ...
    function getStatesByCountry(nal_id){
        ...
        $.ajax({
            ...
            success: function (resp) {
                ...
                $('#lga_id').empty().append($('<option>', { value: '', text: '' }));
                $('#ward').empty().append($('<option>', { value: '', text: '' }));
                $('#street').empty().append($('<option>', { value: '', text: '' }));
            }
        })
    }
    ...
    function getLGA(state_id){
        ...
        $.ajax({
            ...
            success: function (resp) {
                ...
                $('#street').empty().append($('<option>', { value: '', text: '' })).trigger('change');
            }
        })
    }

    function getWards(lga_id){
        var url = '{{ route('get_wards', [':id']) }}';
        url = url.replace(':id', lga_id);
        var ward = $('#ward');
    ...
    function getVillages(ward_id){
        var url = '{{ route('get_villages', [':id']) }}';
        url = url.replace(':id', ward_id);
        var street = $('#street');
...
//        Update Values
    ...
    // Initialize dynamic selects with tags support
    function initDynamicSelects() {
        ...
        $('#street').select2({
            tags: true,
            tokenSeparators: [','],
            placeholder: "Select or Type Street/Village"
        });
    }
