...
@section('page_title', 'Child Attendance')

@section('content')

    <div class="card mb-3">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h6 class="card-title mb-0">Child Attendance</h6>
            <a href="{{ route('my_children') }}" class="btn btn-sm btn-secondary">Back to My Children</a>
        </div>
        <div class="card-body">
            <form method="get" action="{{ route('my_children.attendance') }}">
                <div class="form-row">
                    <div class="col-md-3 mb-2">
                        <label class="font-weight-semibold">Child</label>
                        <select name="child_id" class="form-control select" onchange="this.form.submit()">
                            @foreach($myChildren as $sr)
                                @if($sr->user)
                                    <option value="{{ $sr->user_id }}" {{ $childId == $sr->user_id ? 'selected' : '' }}>
                                        {{ $sr->user->name }} ({{ $sr->adm_no }})
                                    </option>
                                @endif
                            @endforeach
                        </select>
    ...
    <div class="card mb-3">
        <div class="card-header">
            <h6 class="card-title mb-0">Summary @if($child) for {{ $child->name }} @endif</h6>
        </div>
        <div class="card-body">
            <div class="row mb-2">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <p class="mb-2 text-muted">
                @if($child)
                    Showing attendance for <strong>{{ $child->name }}</strong>
                @else
                    Showing attendance for selected child
                @endif
